...
class FamilyController(
    ...
    @GetMapping("/code")
    fun code() = userFacade.getCurrentUser().id

    @Operation(summary = "가족 구성원 생성")
    @PostMapping
    fun add(@RequestBody addRequest: AddRequest) = addService.execute(addRequest)

    @Operation(summary = "가족 구성원 리스트 출력")
    @GetMapping
    fun list(@RequestParam("code") code: String) = familyListService.execute(code)
}